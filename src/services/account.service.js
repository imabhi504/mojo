...
async function login() {
    ...
    const { authResponse } = await new Promise(function(resolve, reject) {
    window.FB.login(
        function(response) {
            resolve(response);
        },
        {
          config_id: '395189936866446' // configuration ID goes here
        }
      )});
...
async function apiAuthenticate(accessToken) {
    ...
    const response = await axios.post(`${baseUrl}/api/facebook/callback/mobile`, { accessToken });
    const account = response.data.result;
    console.log('account',account)
    accountSubject.next(account);
    localStorage.setItem('influencer_id', account.id);
...
async function getAll() {
    ...
    return await axios.get(`${baseUrl}/api/facebook/page_list?influencer_id=${influencer_id}`).then(response => response.data.result);
}
